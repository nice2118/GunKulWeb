            ...
            for ($i = 0; $i < count($inputNames); $i++) {
                if ($inputNames[$i] != '' && $inputValues[$i] != '') {
                    ...
                    if ($inputcode[$i] == 0 || $inputcode[$i] == '') {
                        $sql = "INSERT INTO `engravedactivities` (`EA_Code`, `EC_Code`, `EA_Name`, `EA_Path`, `EC_UserCreate`, `EA_CreateDate`, `EA_ModifyDate`) VALUES (NULL, '$inputMastercode', '$inputNames[$i]', '$inputValues[$i]', '', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);";
                    } else {    
