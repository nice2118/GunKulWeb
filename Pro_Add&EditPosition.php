...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($EC_Code == 0 || $EC_Code == '') {
      $sql = "INSERT INTO `position` (`PT_Code`, `PT_Name`, `PT_CreateDate`, `PT_ModifyDate`) VALUES (NULL, '$PT_name', current_timestamp(), current_timestamp());";
  } else {    
      $sql = "UPDATE `position` SET `PT_Name` = '$PT_name', `PT_ModifyDate` = current_timestamp() WHERE `position`.`PT_Code` = $EC_Code;";
  }
