    ...
    if (isset($_POST['PM_Code'])) {
        ...
        for ($i = 0; $i < count($PM_Code); $i++) {
            if ($PM_Code[$i] == 0 || $PM_Code[$i] == '') {
                ...
                $sql = "UPDATE `permissionmenu` SET `PM_RelationType` = '$PM_RelationType[$i]', `PM_RelationCode` = '$PM_RelationCode[$i]', `PM_Name` = '$PM_Name[$i]', `PM_Direction` = '$PM_Direction[$i]', `PM_Draw` = '$PM_Draw[$i]', `PM_Setup` = '$PM_Setup[$i]', `PM_UserCreate` = '$globalCurrentUser', `PM_ModifyDate` = current_timestamp() WHERE `permissionmenu`.`PM_Code` = '$PM_Code[$i]';";
            }
